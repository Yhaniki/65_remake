...
CSV 欄位：gn, title, artist, src, bpm, offsetMs, fileId
  - 要改名就改 title / artist 兩欄；bpm 欄也可改(覆蓋歌單/房間顯示的 BPM 數字，
    只影響顯示，遊戲判定/流速一律讀譜面本身)。留空 = 不動該首 bpm。
  - offsetMs = 這首歌的音訊校正(毫秒)：正 = 音樂晚一點進來、負 = 提早、0/留空 = 不動。
    這是唯一會真的進遊戲的欄位(只挪音樂＋舞蹈，音符/判定仍讀譜面)。
...
COLS = ["gn", "title", "artist", "src", "bpm", "offsetMs", "fileId"]
...
def export(json_path: Path, csv_path: Path) -> int:
    ...
    print("  用 Excel 打開改 title / artist（offsetMs 欄可填音訊校正毫秒），改完跑： python tools/song_names_csv.py apply")
    return 0
...
def apply(csv_path: Path, json_path: Path) -> int:
    ...
    for row in reader:
        ...
        off_ms = round(float(row["offsetMs"]), 1) if _isfloat(row.get("offsetMs")) else None
        e = by_gn.get(gn)
        if e is None:                       # CSV 新增列 -> 新增到清單
            e = {"gn": gn,
                 ...
                 "offsetMs": off_ms if off_ms is not None else 0.0,
                 "src": (row.get("src") or "manual").strip() or "manual",
                 "title": title, "artist": artist}
            songs.append(e); by_gn[gn] = e; n_add += 1
            continue
        renamed = e.get("title", "") != title or e.get("artist", "") != artist
        changed = renamed
        # bpm 也可改(只影響歌單/房間顯示的 BPM 數字，遊戲判定與流速仍讀譜面)。空白 = 不動。
        if bpm is not None and abs(float(e.get("bpm") or 0.0) - bpm) > 1e-6:
            e["bpm"] = bpm
            changed = True
        # offsetMs = 音訊校正(毫秒)。空白 = 不動。改這欄不算改名 → 不動 src。
        if off_ms is not None and abs(float(e.get("offsetMs") or 0.0) - off_ms) > 1e-6:
            e["offsetMs"] = off_ms
            changed = True
        if not changed:
            n_same += 1
            ...
            e["title"] = title
            e["artist"] = artist
            if renamed and e.get("src") != "manual":   # 標記此列被人改過(除非本來就 manual)
                e["src"] = "manual"
            n_upd += 1
