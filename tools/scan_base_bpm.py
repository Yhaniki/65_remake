...
import song_table                                                     # noqa: E402  （不簡寫成 st：main 裡的 st 是統計 dict）
...
def plain_body(raw: bytes, e: dict):
    """依 song_table 那一列的解密欄（enc/seed/innerOff）把 .gn 解成明文（表頭 + StepFrame body）。"""
    enc = e.get("enc")
    if enc == "sdom":
...
def load_charts(music: Path, diff: int):
    # 一列 = 一個 .gn 檔（k 與 t 各一列），解密欄就在同一列上，不必再跟別的表對照。
    out = []
    for e in song_table.load():
        p = music / e["gn"]
        if not p.exists():
...
def titles():
    """詞幹 → 顯示歌名。k/t 兩列的 title 是同步的，隨便取到哪一列都一樣。"""
    return {song_table.stem(r["gn"]): r["title"] for r in song_table.load()}
