...
PMX_PATH = r"H:/65_remake/assets/MODEL/IkaHatunemiku2025/Ika-HatsuneMiku 2025-JP.Pmx"
    ...
    def solve_substep(self, pos, quat, vel, avl):
        ...
        for cp in p.getContactPoints():
            ...
            if not (self.dynamic[ca] or self.dynamic[cb]):
                continue
            # Enforce the AUTHORED collision filter here. setCollisionFilterGroupMask did NOT take effect for these
            # multibody bases -- pybullet still reports pairs the author disabled (measured: the twintail root gets 10
            # contacts with the head and the hairlines, every one of them filtered off in the .pmx), and those bogus
            # contacts prop the hair up. Two bodies collide iff each has the other's group bit set in its own mask.
            ga, gb = self.pmx.rigid_bodies[ca].group, self.pmx.rigid_bodies[cb].group
            ma, mb = self.pmx.rigid_bodies[ca].mask, self.pmx.rigid_bodies[cb].mask
            if not (((ma >> gb) & 1) and ((mb >> ga) & 1)):
                continue
...
def run_scenario(pmx, name):
    ...
    path = os.path.join(HERE, "ref_%s_filtered.json" % name)
