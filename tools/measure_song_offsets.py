...
用法：
  ...
  python tools/measure_song_offsets.py --apply --max-abs 60  # 確認過分布之後才用：|lag|<=60 且信心夠 → 寫回 overrides
"""
...
DEFAULT_OVERRIDES = SA / "song_name_overrides.json"
...
def main() -> int:
    ...
    ap.add_argument("--apply", action="store_true",
                    help="把結果寫回 song_name_overrides.json 的 offsetMs（**先看過分布再用**）")
    ...
    # ---- --apply：寫回 song_name_overrides.json ----
    ov_path = Path(args.overrides)
    ov = json.loads(ov_path.read_text(encoding="utf-8"))
    by_gn = {str(r.get("gn", "")).lower(): r for r in ov["songs"]}

    set_n = skip_conf = skip_abs = skip_missing = 0
    for r in rows:
        row = by_gn.get(r["gn"].lower())
        if row is None:
            skip_missing += 1
            continue
    ...
    print(f"  設定 {set_n} 首；跳過：信心不足/打點太少 {skip_conf}、|lag| > {args.max_abs}ms {skip_abs}、"
          f"overrides 裡沒有這首 {skip_missing}")
    return 0
