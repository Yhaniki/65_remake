# -*- coding: utf-8 -*-
"""
gn_keytable — 預先把 music 目錄下所有 .gn 的解密金鑰(seed)算好，寫成一張 JSON 金鑰表。
...
支援的 .gn 類型(由解密引擎自動判別，對齊 H:/bms/tools/bms_sdo/gn_crypto.py):
  ...
  rewu  : 熱舞 Online 整檔 LCG      — (本資料夾未出現；偵測到會標記但不破解)
...
用法:
  ...
  python tools/gn_keytable.py <root> -o <out.json> [--recursive] [--limit N]
"""
...
def main() -> int:
    if hasattr(sys.stdout, "reconfigure"):
        try:
            ...
            pass
    ap = argparse.ArgumentParser(description="建立 .gn 金鑰表 (JSON)")
    ap.add_argument("root", help="music 目錄")
    ap.add_argument("-o", "--output", help="輸出 JSON（預設 <root>/gn_keytable.json）")
    ...
    if not root.is_dir():
        ...
        return 1
    out_path = Path(args.output).resolve() if args.output else root / "gn_keytable.json"
    ...
    # 陣列式 schema：對齊 StreamingAssets/song_catalog.json，Unity JsonUtility 可直接讀。
    # 每筆固定欄位順序，gn 用小寫檔名(與 SongCatalog 的鍵一致)。
    field_order = ("enc", "mode", "seed", "innerOff", "seed1", "seed2",
    ...
    out = {
        ...
        "generatedBy": "tools/gn_keytable.py",
        "root": root.name,
        "recursive": args.recursive,
        "multiplier": "0x3D09",
        "howto": {
            "sdom": "body = lcg_decrypt(seed, raw[innerOff+300:]); stepfileHeader = raw[innerOff:innerOff+300]",
            "ddrm": "DDRM: lcg_decrypt(seed1, raw[0x20:0x40]) 取 seed2; body = lcg_decrypt(seed2, raw[0x54:])",
            "plain": "raw 即 StepFile，無需解密",
    ...
    dt = time.time() - t0
    print(f"完成：{out_path}")
    print(f"  耗時 {dt:.1f}s；類型統計 {out['counts']}")
