...
它是既有 CLI 工具的殼,不是第二套邏輯:
    ...
    改名 = 寫 song_name_overrides.json(runtime 疊在 song_catalog 上的顯示層)
...
def write_overrides(doc: Dict) -> None:
    """寫回 overrides + 重生 CSV 鏡像(跟 remove_songs 一樣,免得兩邊不同步)。"""
    doc["count"] = len(doc.get("songs", []))
    OVERRIDES_JSON.write_text(json.dumps(doc, ensure_ascii=False, indent=1) + "\n", encoding="utf-8")
    try:
        with OVERRIDES_CSV.open("w", encoding="utf-8-sig", newline="") as f:
            w = csv.DictWriter(f, fieldnames=CSV_COLS, extrasaction="ignore")
            w.writeheader()
            for e in doc.get("songs", []):
                w.writerow({c: e.get(c, "") for c in CSV_COLS})
    except OSError:
        pass          # CSV 只是給人看的鏡像,寫不了不影響遊戲


class Song:
    """一首歌(K/T 兩譜合成一列),顯示值已照 runtime 規則套過 overrides。"""

    def __init__(self, st: str, cat: Dict, ov: Optional[Dict]):
        self.stem = st
        self.file_id = int(cat.get("fileId", 0))
        self.gns: List[str] = list(cat.get("gns", []))
        self.title = cat.get("title", "")
        self.artist = cat.get("artist", "")
        self.bpm = float(cat.get("bpm", 0) or 0)
        # runtime 的套用規則(SongCatalog.ApplyOverrides):空字串不蓋、bpm<=0 不蓋。
        if ov:
            if ov.get("title"):
                self.title = ov["title"]
            if ov.get("artist"):
                self.artist = ov["artist"]
    ...
    def assets(self) -> str:
        ...
        f = self.file_id
        marks = [
            ("♪", (MUSIC / f"{self.stem}.ogg").is_file()),                    # 主音樂(缺 = 沒聲音)
            ("▶", (EXPER / f"{f}.ogg").is_file()),                            # 試聽(缺 = 退主音樂中段)
            ("◆", (DANCE / f"{f}.DPS").is_file()),                            # 舞蹈(缺 = 通用舞)
            ("★", (ICON_OVERLAY / f"{f}.PNG").is_file()
                  or (ICON_OVERLAY / f"{f}.png").is_file()),                  # 封面(缺 = 佔位圖)
        ]
        return "".join(m if ok else "·" for m, ok in marks)


def load_songs() -> List[Song]:
    by_stem = load_catalog()                       # {stem: {fileId,title,gns}} — 以 K 譜為準
    cat_rows = json.loads(CATALOG.read_text(encoding="utf-8")).get("songs", [])
    bpm_by_stem = {stem(r["gn"]): r.get("bpm", 0) for r in cat_rows if r.get("gn", "").lower().endswith("k.gn")}
    ov = {e["gn"]: e for e in read_overrides().get("songs", []) if e.get("gn")}
    songs = []
    for st, cat in by_stem.items():
        cat = dict(cat, bpm=bpm_by_stem.get(st, 0))
        songs.append(Song(st, cat, ov.get(st)))
...
def used_slots() -> tuple[set[str], set[int]]:
    """已用的詞幹 + fileId。同時看目錄表「和」music 目錄實體檔 —— 目錄表沒收進去的殘骸
    也算占用,否則新歌會覆蓋掉它。"""
    stems, fids = set(), set()
    for e in json.loads(CATALOG.read_text(encoding="utf-8")).get("songs", []):
        stems.add(stem(e.get("gn", "")))
        fids.add(int(e.get("fileId", 0)))
    for p in MUSIC.glob("*.gn"):
...
def add_song(*, k: Path, stem_: str, file_id: int, ogg: Path, t: Optional[Path] = None,
    ...
    """加一首歌:驗號 → 驗譜面解得開 → 複製改名 → 更新四張表 → (可選)寫歌名/BPM。
    ...
    try:
        ...
        k_dst = MUSIC / f"{st}K.gn"
        cp(k, k_dst)
        t_dst = None
        if t:
            t_dst = MUSIC / f"{st}T.gn"
            cp(t, t_dst)
        cp(ogg, MUSIC / f"{st}.ogg")
        for src, dst in ((exper, EXPER / f"{file_id}.ogg"),
                         (dps, DANCE / f"{file_id}.DPS"),
                         (icon, ICON_OVERLAY / f"{file_id}.PNG")):
            if src:
                cp(src, dst)
...
class App(tk.Tk):
    ...
    def refresh(self):
        ...
        cat = {e["gn"]: e for e in json.loads(CATALOG.read_text(encoding="utf-8")).get("songs", [])}
        for i, s in enumerate(self.shown):
            row = next((cat[g] for g in s.gns if g.endswith("k.gn")), {})
    ...
    def on_delete(self):
        ...
        if not messagebox.askyesno(
                "確定移除?",
                f"會刪掉這些歌的檔案(譜面/主音樂/試聽/舞蹈/封面)並從 4 張目錄表移除:\n\n{names}{more}\n\n"
