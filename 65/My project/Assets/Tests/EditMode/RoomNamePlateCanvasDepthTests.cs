...
{
    /// <summary>
    /// 房間頭上名字牌能被前面的人/牆/家具擋住,靠的是一個 Unity 行為假設:**world-space canvas 被透視相機
    /// render 時會做深度測試**(UI 材質的 ZTest 不是 Always)。這裡把那個假設釘住 —— 尤其是 **TMP 的字**,
    /// 因為字是整面名字牌最容易出問題的部分:
    ///
    ...
    ///
    /// 所以「圖會被擋住」不代表「字也會被擋住」,而症狀會是:只有名字裡帶罕見字的那個人,
    /// 一部分字穿透牆壁/人體 —— 幾乎不可能從畫面猜到原因。
    ///
    /// 這裡不依賴任何專案程式碼的定位數學(那由 <see cref="RoomNamePlateAnchorTests"/> 釘),純粹驗 Unity 的性質。
    /// </summary>
    public class RoomNamePlateCanvasDepthTests
    {
        ...
        public void SetUp()
        {
            ...
            var camGo = new GameObject("namePlateDepthProbeCam");
            ...
            _canvas = UIKit.CreateDepthTestedWorldCanvas("namePlateDepthProbeCanvas", null, Layer, new Vector2(171f, 111f));
            _canvasGo = _canvas.gameObject;
            _canvas.pivot = new Vector2(0.5f, 0.5f);   // 這個測試要牌子在畫面正中央,方便與 cube 重疊
            ...
            UIKit.SetLayerRecursive(_canvasGo, Layer);
        }
        ...
        [Test]
        public void Geometry_In_Front_Occludes_Both_The_Nameplate_Art_And_Its_Text()
        {
            // A:遮擋物在名字牌**後面** → 牌子完整看得到,遮擋物一個像素都不該露出來。
            _cube.transform.position = new Vector3(0f, 0f, 150f);
            var a = Shoot();
            int aArt = CountGreen(a), aText = CountWhite(a), aCube = CountRed(a);
            Assert.Greater(aArt, 5000, "名字牌的底圖沒畫出來");
            Assert.Greater(aText, 100, "名字沒畫出來(字是 TMP,材質與圖不同 → 要單獨驗)");
            Assert.AreEqual(0, aCube, "遮擋物在名字牌後面,不該有任何像素露出來");

            // B:遮擋物在名字牌**前面** → 圖與字都必須被切掉一部分。這正是使用者要的
            //    「後面那個人的名牌不該出現在前面那個人的身體前面」。
            ...
            Assert.Less(bArt, aArt * 0.95f, "名字牌沒有被前面的東西切掉 —— world canvas 沒吃到深度測試");
            Assert.Less(bText, aText * 0.95f,
                "名字牌的**字**沒有被切掉(圖有、字沒有)—— TMP 的材質沒吃到 canvas 的 ZTest 設定。"
                + "這種狀況下牌子看起來被擋住,但名字會穿透人體。");
        }

        [Test]
        public void Tmp_Fallback_Submeshes_Inherit_The_Nameplate_Layer()
        {
            // 字型是執行期 OS SimSun;`𠀋`(CJK ext-B)不在 SimSun 裡 → TMP 會為後備字型長出 sub-mesh 子物件。
            // 那些子物件**必須**落在名字牌的 layer 上,否則它們會從房間 RT 消失、或被前端 UI 相機畫在錯的位置。
            _label.text = "口𠀋口";
        ...
        {
            Assert.AreEqual(Layer, t.gameObject.layer, "後代物件 " + t.name + " 不在名字牌的 layer 上");
