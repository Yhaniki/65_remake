namespace Sdo.Net
{
    ...
    public sealed class NetRoomSnapshot
    {
        ...
        /// 一個小數字(第幾間房),不是 5 位數房號。由 server 配「**目前沒人用的最小號、從 0 起算**」——
        /// 關掉的房間門牌會被下一間房拿去用(見 <c>RoomRegistry.NextFreeSeq</c>),
        /// 因為它是版面上的格子位置,不是流水號。官方實機大廳第一格就是 <c>000</c>。
        ///
        /// 與 <see cref="Code"/> 是**兩件不同的事**:Code 是加入房間的鑰匙(全域唯一、5 位數),
        /// Seq 只是給人看的門牌號(小、好唸、可能在 server 重啟後從 000 重新算)。
        /// <b>0 是一間真的房</b> —— 「不在任何房裡」的哨兵值是 -1(見 <c>NetUserListEntry.InLobby</c>)。
        /// </summary>
        public int Seq;
