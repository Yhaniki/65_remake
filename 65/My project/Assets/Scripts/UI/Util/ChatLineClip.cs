...
namespace Sdo.UI.Util
{
    ...
    ///
    /// RectMask2D 是逐像素裁切,不管行界:視窗高度不是行高的整數倍時,捲到底的視窗上緣會落在某一行的中間
    /// → 最上面那行只露出下半截字,而且因為永遠釘在底部,這截殘影會一直留著。
    /// 這裡在排版/捲動之後把「沒有整行落在視窗內」的行 alpha 歸零;用 CanvasGroup 而不是 SetActive,
    /// 才不會把它踢出 VerticalLayoutGroup 而改變內容高度、反過來又動到捲動位置。
    ///
    /// (大廳的聊天區現在是 110px 視窗 / 11px 行高 + 1px 間距 → 剛好 9 整行,見 LobbyScreen.ChatLineH。
    ///  這支元件仍然要留著 —— 長訊息會折行,行高不再是固定值。)
    /// </summary>
    ...
    public sealed class ChatLineClip : MonoBehaviour
    {
        ...
        private float _lastViewHeight = float.NaN;
        ...
        private void Apply()
        {
            ...
            // 只有捲動位置 / 內容高度 / 行數 / 視窗高度變了才重算(訊息欄每幀都在,行數可能上百)。
            // 視窗高度也要看:少了它,面板改尺寸之後這裡會沿用上一次算的結果,半截字就留在畫面上。
            float y = content.anchoredPosition.y, h = content.rect.height, vh = vp.rect.height;
            int n = content.childCount;
            if (n == _lastCount && Mathf.Approximately(y, _lastY) && Mathf.Approximately(h, _lastHeight)
                && Mathf.Approximately(vh, _lastViewHeight)) return;
            _lastY = y; _lastHeight = h; _lastCount = n; _lastViewHeight = vh;
