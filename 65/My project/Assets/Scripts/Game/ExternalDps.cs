...
namespace Sdo.Game
{
    ...
    /// fresh one.)
    ///
    /// 🔴 <c>#DPS</c> 也可以**手寫**:把自己編的 .dps(連同它點名的 .mot,那些 clip 在歌曲資料夾裡優先於資料樹,
    /// 見 <c>ScreenGameplay.TryLoadMotFromSongFolder</c>)丟進歌曲資料夾,在 sidecar 寫
    /// <c>#DPS:&lt;檔名&gt;;</c> 即可 —— 只要檔名不是我們生成的那一套(<see cref="SongSidecar.IsGeneratedDpsName"/>),
    /// 它就永遠照跳:不看 <c>#DPSVER</c>、不重生成、也不會被改寫。離線與連線走的是同一段程式,兩邊一致。
    ///
    ...
    public static class ExternalDps
    {
                                       ...
                                       IReadOnlyList<string> chartPaths, IReadOnlyList<int> chartIndices)
        {
            ...
            if (recorded != null && !string.IsNullOrEmpty(recorded.Dps))
            {
                string had = Path.Combine(folderPath, recorded.Dps);
                bool ours = SongSidecar.IsGeneratedDpsName(recorded.Dps);
                // 🔴 使用者手寫的 #DPS(指向他自己放進資料夾的客製 .dps)一律照用,**不看 #DPSVER**:那個版本號
                // 的意思是「我這一版產生器造的」,對別人手寫的檔案沒有發言權。以前一視同仁的結果是,手寫
                // 「#DPS:12951.DPS;」而沒補「#DPSVER:5;」→ 版本讀成 0 = 舊版 → 第一次開這首歌就被重生成的
                // dance.dps 覆寫掉,客製編舞(連同它點名的客製 .mot)從此再也沒被跳過,而且離線/連線都一樣。
                if (File.Exists(had))
                {
                    if (!ours) Debug.Log($"[dps] {Path.GetFileName(folderPath)}: 用歌曲資料夾自帶的編舞 {recorded.Dps}(sidecar #DPS 指名)");
                    if (!ours || recorded.DpsVersion >= SongSidecar.DpsGenerator) return had;
                }
                else if (!ours)
                {
                    // 指名了卻找不到檔 —— 多半是打錯字。講清楚是哪一支,否則使用者只會看到「還是生成的舞」。
                    SdoLog.Note("dps", "sidecar #DPS 指名的 " + recorded.Dps + " 不在 " + folderPath + " 裡 → 改用生成的舞");
                }
            }
            // 只有**我們自己生的**舞會被版本汰換:產生器修好了,已經跳過的歌也要吃到(seed 沒變,還是這首歌自己的舞)。
