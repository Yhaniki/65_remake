...
namespace Sdo.Game
{
    ...
    ///   UI/MATCHITEMS           寵物圖 (DRESS: 1000001 → 1000001.an)
    /// 3D 的部分：道具/禮盒在 <c>DAOJU/</c>、寵物與寵物頭飾在 <c>PETAVATAR/</c>。
    ...
    public static class DressCatalog
    {
        ...
        // 2D 圖示資料夾 (優先序:商城限定 → 一般 → 使用中 → 寵物)
        private static readonly string[] IconDirs =
        {
            "UI/ITEM2D_PACK_IN_SHOP", "UI/ITEM2D_PACK", "UI/ITEM2D_PACKUSE", "UI/MATCHITEMS",
        };
        // 3D 模型資料夾 (寵物/寵物頭飾 → PETAVATAR;道具/禮盒 → DAOJU)
        private static readonly string[] MeshDirs = { "PETAVATAR", "DAOJU" };
        ...
        /// <summary>3D 模型的 data-root 相對路徑 (如 <c>PETAVATAR/1030000_ALL_HEADEAR.MSH</c>)。不是 .msh → null。
        /// 寵物衣服 (只有貼圖) 走 <see cref="PetClothesMesh"/>。</summary>
        ...
        /// <summary>寵物衣服:全部共用同一件 coat mesh、只換自己的貼圖 → 回傳 (mesh 相對路徑, 貼圖檔名)。不是寵物衣服 → false。</summary>
        public static bool PetClothesMesh(int modelId, out string meshRel, out string ddsName)
        {
            meshRel = null; ddsName = null;
            if (!DressTable.IsPetClothes(modelId)) return false;
            var dds = Resource(modelId);
            if (DressTable.KindOf(dds) != DressResourceKind.Texture) return false;
            var shared = Resource(DressTable.PetCoatMeshKey);            // 1041000 → 1040000_all_coat_.msh
            var full = FindIn(MeshDirs, shared, DressResourceKind.Mesh);
            if (full == null) return false;
            meshRel = RelOf(full); ddsName = dds;
            return true;
        }
