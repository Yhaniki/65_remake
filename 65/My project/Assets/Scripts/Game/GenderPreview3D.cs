...
namespace Sdo.Game
{
    ...
    public sealed class GenderPreview3D : MonoBehaviour
    {
        ...
        private Transform BuildAvatar(bool male, string name)
        {
            ...
            // MMD 顯示模式下這張預覽也跟著換。Register BEFORE parking it inactive, so a build that's needed
            // right now (MMD already on) happens on a live GameObject; MmdAvatarSwap retries the other gender when it's shown.
            // The MMD rig is height-matched to the SDO body, so the head-to-toe framing below needs no MMD-specific case.
            MmdAvatarSwap.Register(av);
            go.SetActive(false);
            return go.transform;
        }
