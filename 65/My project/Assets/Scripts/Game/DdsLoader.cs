...
namespace Sdo.Game
{
    ...
    public static class DdsLoader
    {
        ...
        /// <summary>
        /// Decode a DDS to RGBA32 pixels in the FILE's row order (row 0 = the image's TOP row) plus its size — the raw
        /// material for a caller that must READ or TRANSFORM the pixels, chiefly a full-rect UI Sprite (a song's .dds CD
        /// disc): a Unity texture's row 0 is the BOTTOM, so such a caller has to flip the rows itself, which the mesh path
        /// <see cref="Load(byte[])"/> never can (it uploads a non-readable, un-flipped texture). Handles the four layouts a
        /// cover can arrive in — DXT1 (with real BC1 punch-through alpha, so a round disc's corners key out), DXT3, DXT5,
        /// and 32-bit uncompressed A8R8G8B8/X8R8G8B8 — and returns null for anything else (base mip only). Reuses the same
        /// block decoders as <see cref="Load(byte[])"/> via the shared helpers. Pure (byte[] -> Color32[]), so it is
        /// unit-tested independently of any Texture2D.
        /// </summary>
        public static Color32[] DecodeToPixels(byte[] d, out int width, out int height)
        {
            width = height = 0;
            if (d == null || d.Length < 128 || d[0] != 'D' || d[1] != 'D' || d[2] != 'S' || d[3] != ' ') return null;
            int h = BitConverter.ToInt32(d, 12), w = BitConverter.ToInt32(d, 16);
            ...
            switch (fourcc)
            {
                case "DXT1": px = Dxt1Pixels(d, 128, w, h); break;
                case "DXT3": px = Dxt3Pixels(d, 128, w, h); break;
                case "DXT5": px = Dxt5Pixels(d, 128, w, h); break;
                default:
                    uint pf = BitConverter.ToUInt32(d, 80);
                    if ((pf & 0x4u) != 0 || (pf & 0x40u) == 0) return null;   // not uncompressed DDPF_RGB
                    px = UncompressedPixels(d, 128, w, h,
                        BitConverter.ToUInt32(d, 92), BitConverter.ToUInt32(d, 96),
                        BitConverter.ToUInt32(d, 100), BitConverter.ToUInt32(d, 104),
        ...
        // BC1 → RGBA32, HONOURING the 1-bit punch-through alpha (c0<=c1 blocks' 4th index → alpha 0). Unlike the stage
        // path (Load()'s forced-opaque DecodeDxt1), a disc's DXT1 corners are genuinely cut out. Top row first.
        private static Color32[] Dxt1Pixels(byte[] d, int off, int w, int h)
        {
            int bw = (w + 3) / 4, bh = (h + 3) / 4;
            if (off + bw * bh * 8 > d.Length) return null;
            var px = new Color32[w * h];
            ...
            for (int by = 0; by < bh; by++)
                for (int bx = 0; bx < bw; bx++, bi += 8)
                {
                    ushort c0 = (ushort)(d[bi] | (d[bi + 1] << 8)), c1 = (ushort)(d[bi + 2] | (d[bi + 3] << 8));
                    uint bits = (uint)(d[bi + 4] | (d[bi + 5] << 8) | (d[bi + 6] << 16) | (d[bi + 7] << 24));
                    Color32 p0 = From565(c0), p1 = From565(c1), p2, p3;
                    if (c0 > c1)
                    {
                    ...
                    {
                        p2 = new Color32((byte)((p0.r + p1.r) / 2), (byte)((p0.g + p1.g) / 2), (byte)((p0.b + p1.b) / 2), 255);
                        p3 = new Color32(0, 0, 0, 0);   // BC1 transparent index
                    }
                    for (int i = 0; i < 16; i++)
                    {
                        int x = bx * 4 + (i & 3), y = by * 4 + (i >> 2);
        ...
        // BC2: 8 bytes 4-bit explicit alpha + 8 bytes DXT1-style colour (always 4-colour). Top row first.
        private static Color32[] Dxt3Pixels(byte[] d, int off, int w, int h)
        {
            int bw = (w + 3) / 4, bh = (h + 3) / 4;
            if (off + bw * bh * 16 > d.Length) return null;
            var px = new Color32[w * h];
            ...
            for (int by = 0; by < bh; by++)
                for (int bx = 0; bx < bw; bx++, bi += 16)
                {
                    ushort c0 = (ushort)(d[bi + 8] | (d[bi + 9] << 8)), c1 = (ushort)(d[bi + 10] | (d[bi + 11] << 8));
                    uint bits = (uint)(d[bi + 12] | (d[bi + 13] << 8) | (d[bi + 14] << 16) | (d[bi + 15] << 24));
                    Color32 p0 = From565(c0), p1 = From565(c1);
                    Color32 p2 = new Color32((byte)((2 * p0.r + p1.r) / 3), (byte)((2 * p0.g + p1.g) / 3), (byte)((2 * p0.b + p1.b) / 3), 255);
                    Color32 p3 = new Color32((byte)((p0.r + 2 * p1.r) / 3), (byte)((p0.g + 2 * p1.g) / 3), (byte)((p0.b + 2 * p1.b) / 3), 255);
                    for (int i = 0; i < 16; i++)
                    {
                        ...
                        int sel = (int)((bits >> (i * 2)) & 3);
                        Color32 col = sel == 0 ? p0 : sel == 1 ? p1 : sel == 2 ? p2 : p3;
                        int ab = d[bi + (i >> 1)], nib = (i & 1) == 0 ? (ab & 0xF) : ((ab >> 4) & 0xF);
                        px[y * w + x] = new Color32(col.r, col.g, col.b, (byte)((nib * 255 + 7) / 15));
                    }
                }
            return px;
        ...
        // BC3: 8 bytes interpolated alpha (a0,a1 + 3-bit codes) + 8 bytes DXT1-style colour (always 4-colour). Top row first.
        private static Color32[] Dxt5Pixels(byte[] d, int off, int w, int h)
        {
            int bw = (w + 3) / 4, bh = (h + 3) / 4;
            if (off + bw * bh * 16 > d.Length) return null;
            var px = new Color32[w * h];
            ...
            for (int by = 0; by < bh; by++)
                for (int bx = 0; bx < bw; bx++, bi += 16)
                {
                    ...
                    for (int k = 0; k < 6; k++) aBits |= (ulong)d[bi + 2 + k] << (8 * k);
                    int co = bi + 8;
                    ushort c0 = (ushort)(d[co] | (d[co + 1] << 8)), c1 = (ushort)(d[co + 2] | (d[co + 3] << 8));
                    uint bits = (uint)(d[co + 4] | (d[co + 5] << 8) | (d[co + 6] << 16) | (d[co + 7] << 24));
                    Color32 p0 = From565(c0), p1 = From565(c1);
                    Color32 p2 = new Color32((byte)((2 * p0.r + p1.r) / 3), (byte)((2 * p0.g + p1.g) / 3), (byte)((2 * p0.b + p1.b) / 3), 255);
                    Color32 p3 = new Color32((byte)((p0.r + 2 * p1.r) / 3), (byte)((p0.g + 2 * p1.g) / 3), (byte)((p0.b + 2 * p1.b) / 3), 255);
                    for (int i = 0; i < 16; i++)
                    {
                        ...
                        int sel = (int)((bits >> (i * 2)) & 3);
                        Color32 col = sel == 0 ? p0 : sel == 1 ? p1 : sel == 2 ? p2 : p3;
                        int a = Dxt5Alpha(a0, a1, (int)((aBits >> (i * 3)) & 7));
                        px[y * w + x] = new Color32(col.r, col.g, col.b, (byte)a);
                    }
                }
            return px;
        }

        // Uncompressed 32-bit DDS via the pixel-format channel masks → RGBA32 (Amask 0 → opaque). Top row first.
        private static Color32[] UncompressedPixels(byte[] d, int off, int w, int h, uint rm, uint gm, uint bm, uint am, int bits)
        {
            if (bits != 32 || off + w * h * 4 > d.Length) return null;
            int rs = MaskShift(rm), gs = MaskShift(gm), bs = MaskShift(bm), as_ = MaskShift(am);
            var px = new Color32[w * h];
            for (int i = 0; i < w * h; i++)
            {
