...
### `comboMilestone` —— 去重是「不同的值」,不是「遞增」

combo 每 50 一個里程碑(頭上的表情、每 100 一次腳下的爆發)。要獨立傳是因為分數流只有 5 Hz:
收端看到的 combo 會直接從 49 跳到 53,「剛好踩到 50」在快照裡看不到。

🔴 **server 的去重只能擋「同一則被重送」,不可以要求遞增。** combo 會斷 —— 打到 250 之後
Bad 一下歸零,再爬回 50 / 100 / 150 每一個都是真的里程碑,本機那台也確實會各放一次特效
(`ScreenGameplay.Effects` 的 `_lastMilestone` 只擋「與上一次相同的值」)。用單調遞增去重的話,
斷 combo 之後的那一整段在別人畫面上會完全消失,直到他重新超過斷掉前的最高點 ——
症狀就是使用者回報的「遠端的人 combo 特效有時候沒出來」。判準與防洪門檻在
`Sdo.Net.ComboMilestoneRules`(client 與 server 編譯同一份)。
