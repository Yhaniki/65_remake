# 外部專案參考

非 SDO 反編譯的第三方借鑑索引。

| 專案 | 連結 | 借鑑範圍 |
|------|------|----------|
| **osu lazer** | [github.com/ppy/osu](https://github.com/ppy/osu) | Ruleset、mania 譜、skin、1e8 計分 |
| **SM 3.9 YHANIKI** | [github.com/Yhaniki/SM-YHANIKI](https://github.com/Yhaniki/SM-YHANIKI) | NoteSkin、`.sm`、逆流 scroll、總按鍵數、連線 |
| **咕盒 gogh** | [gogh.gg/zh-sc](https://gogh.gg/zh-sc) | 個人房：虛擬形象、小房間、氛圍 |

## 文件

- SM 對照詳表：[sm-yhaniki-notes.md](sm-yhaniki-notes.md)
- 個人房：[architecture/personal-room-gogh.md](../architecture/personal-room-gogh.md)
- 技術棧：[architecture/stack.md](../architecture/stack.md)

## SDO 原版考據

見 [reverse-engineering/](../reverse-engineering/README.md)
