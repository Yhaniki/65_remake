...
> **檔名對不上時的後備 (`DressCatalog.IconPath`)** —— 實測有 7 個 modelId 照 DRESS.TXT 找不到檔：
> ① 表寫 `101241_fantuan41.an`，磁碟上是 `101241_FANTUAN41_.AN`(多一個底線；同名 `.DDS/.PNG` 反而沒底線)；
> ② 表裡**根本沒有**這筆 —— `1120000 神奇大力丸` / `1120001-2 宝宝培育药水(男/女)` / `1120003 大草莓蛋糕` /
> `1120004 超级棒棒糖` / `1120005 奇妙冰激凌`(7 碼 modelId，DRESS.TXT 只收 6 碼那批)，但
> `UI/ITEM2D_PACK/` 有 `1120000_SHENQIDALIWAN.AN` 等整組。
> → 官方檔名查無時改用 `{modelId}_*.an` 在同一批資料夾撿 (id 後面一定接底線，不會跨 id 誤撿)；資料夾清單有快取。
...
> **道具 mesh 不是站好的** —— `100400_LIHE.MSH`(28 件禮包全借它) 的盒面法線離世界軸約 30°，照原樣畫就是一顆
> 歪倒的方塊、蝴蝶結還轉去背面。扶正靠離線量測：解 MSH 面法線後依方向分群 (面積加權平均)，盒子得到 3 對 —
> `±(0.290,-0.321,0.901)` 面積最大 = 盒蓋/盒底(蝴蝶結長在這面)、`±(-0.308,0.864,0.398)` 與 `±(0.903,0.399,-0.156)` = 四個側面。
> 把盒蓋轉到 +Y，再套跟衣物同一個 30° 側轉 + 前傾 25°(卡片相機是水平正交，要壓下模型才看得到盒蓋)，
> 就是官方那顆「盒子站著、蝴蝶結在上面」的禮盒 (`ShopScreen.PropUprightFor` / `LiheLidNormal`)。
...
## 3. 實測覆蓋率 (以本 worktree 的 DATA 解析)

**上架的**：

| category | 分頁 | 件數 | 解得到美術 | 畫法 |
|---|---|---:|---:|---|
| 21000 | 道具店 · 功能道具 | 188 | 188 (100%) | 2D 圖示 (其中 7 個靠 `{modelId}_*.an` 後備才找到) |
...
| 14000 | 礼包店 | 28 | 28 (100%) | 3D：按 modelId 區間借禮盒模型 (官方寫死)，全部落在 `100400_lihe.msh` |
| **合計** | | **273** | **273 (100%)** | |

(件數 = 不重複 modelId；消耗品的 ×1/×5/×10 SKU 各一張卡，所以畫面上的卡片數更多。)
解不到美術的商品**仍然上架**(使用者要求「都上架」)，只是格子沒有縮圖 —— 目前上架的這 273 件已全部有圖。

**不上架的 — 寵物 (使用者指定)**：重製版沒有寵物系統 (跟隨/餵食/換裝都沒有)，賣了不能用 →
伙伴店四個子分頁照官方座標/美術擺著但點了沒反應。iteminfo 的資料仍照常解析 (`AvatarItemCatalog.Props`)，
之後要做寵物系統時只要把 `ShopScreen.ShoppetTabs` 的 `EquipSlot.None` 換回對應 slot 即可：

| category | 分頁 | 件數 | 美術來源 (已查證) |
|---|---|---:|---|
| 41000 | 宠物 | 52 | 2D 圖示 `UI/MATCHITEMS`(29/52；23 隻舊寵物不在 DRESS.TXT) |
| 43000 | 宠物衣服 | 24 | 3D：31 件共用 `PETAVATAR/1040000_all_coat_.msh` + 各自印花 DDS |
| 42000 | 宠物头饰 | 21 | 3D：`PETAVATAR/*_HEADEAR.MSH`(19/21) |
| 44000 | 宠物道具 | 10 | 2D 圖示 (餅乾…) |
